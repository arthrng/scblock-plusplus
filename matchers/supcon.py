...
import sys
sys.path.append('.')
from matchers.supconmatch.model import ContrastiveClassifierModel
...
class SupConMatcher(Matcher):
    ...
    def __init__(self, schema_org_class, model_path, base_model, context_attributes=None, matcher=False, max_length=128):
        ...
        if model_path is not None:
            ...
            self.model = ContrastiveClassifierModel(len_tokenizer=len(self.tokenizer), checkpoint_path=model_path, model=base_model).to(self.device)
    ...
    def predict_matches(self, entities1, entities2, excluded_attributes1=None, excluded_attributes2=None):
        ...
        records_encoded['input_ids_left'] = records_encoded.pop('input_ids')
        records_encoded['attention_mask_left'] = records_encoded.pop('attention_mask')
        ...
        with torch.no_grad():
            logits = self.model(**records_encoded)
