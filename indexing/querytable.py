...
import sys
sys.path.append('.')
from indexing.evidence import RetrievalEvidence, AugmentationEvidence
...
def get_query_table_paths(path, gt_table):
    """Get query table s"""
    path_to_query_tables = f'{path}/{gt_table}'
    
    return path_to_query_tables
...
class BaseQueryTable:
    ...
    def determine_path_to_querytable(self):
        ...
        return f'./data/querytables/{self.schema_org_class}/{file_name}'
